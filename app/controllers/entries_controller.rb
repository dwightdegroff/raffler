# Raffler application worked correctly like the screencast.
...
class EntriesController < ApplicationController
  ...
  def winner
    # Reset previously chosen winner(s)
    @entries = Entry.all
    @entries.each do |e|
      if e[:winner] 
        e[:winner]= false
    ...
    respond_to do |format|
      if @entry.update_attributes(params[:entry])
        format.html { redirect_to entries_path, notice: @entry.name + ' entry was made a winner.' }
        format.json { render json: @entry, status: :created, location: @entry }
      else
